        ...
        if (name != null && (!name.equals("")) && password != null && (!password.equals(""))){//判断登录用户名密码不为空
            ...
            if (list != null){//list不为空
                ...
                if ((user1.getPassword()).equals(ssr)){//数据库中加密的密码和登录加密的密码一致
                    ...
                    String token= util.JJwtUtils.createToken(user1.getId(),user1.getCreateat(),360000);//
                    CookieController.createCookie("name",token,response);
//                    String cookie = CookieController.createCookie(user1.getName(),user1.getPassword());
//                    CookieController.createCookie("name",user1.getPassword(),response);
//                    String token= utils.JJwtUtils.createToken(user1.getId(),user1.getCreateat(),360000);
//                    CookieController.createCookie(getMD5(name),token,response);
...
//        UserExample example = new UserExample();//新建一个example对象
//        UserExample.Criteria criteria = example.createCriteria();//新建条件
//        criteria.andNameLike(request.getLocalName());//获取姓名查询
