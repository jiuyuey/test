...
//        logger.info(verification);
        ...
        if(list != null) {
            ...
            if ((user1.getPassword()).equals(ssr)) {//数据库中加密的密码和登录加密的密码一致
                String token = JJwtUtils.createToken(user1.getId(), user1.getCreateat(), 360000);//生成一个session
    ...
    public ModelAndView registered(String name, String password,String phone, String email){
    ...
    public ModelAndView validation(String phone, String email, HttpServletRequest request) throws IOException {
        ...
        if (PhoneFormatCheckUtils.isMobileNO(phone)){//判断是不是手机
            ...
            session.setAttribute("moden",phone);
            va.addObject("message","发送成功");
        }else{
        ...
        if (PhoneFormatCheckUtils.checkEmail(email)){//判断是不是邮箱
            ...
            HttpSession session = request.getSession();
            session.setAttribute("code",a);
            session.setAttribute("moden",email);
            va.addObject("message","发送成功");
        }else{
            va.addObject("message","失败");
        }
        va.setViewName("login");
        return va;
    }
